...
默认每 **10 分钟**检测一次所有站点。可在后台设置中调整检测间隔。
...
| 监控类型 | 检测方式 | 检测频率 | 说明 |
|----------|----------|----------|------|
| HTTP/HTTPS | Cron 主动检测 | 每 10 分钟 | 默认间隔，可在后台设置调整 |
| DNS | Cron 主动检测 | 每 10 分钟 | 通过 1.1.1.1 DoH API |
| TCP | Cron 主动检测 | 每 10 分钟 | 通过 Cloudflare Sockets |
...
| 监控站点数 | 每 10 分钟检测 | 估算每日写入 | 配额占用 |
|-----------|---------------|-------------|----------|
| 10 个站点 | 10 次 | ~3,000 次 | 3% |
| 50 个站点 | 50 次 | ~15,000 次 | 15% |
| 100 个站点 | 100 次 | ~30,000 次 | 30% |
| 300 个站点 | 300 次 | ~90,000 次 | 90% |
