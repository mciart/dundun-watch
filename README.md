...
<details>
<summary><b>⚠️ 重要模块化架构重构说明（点击展开）</b></summary>
...
| MQTT   | connect()                         | 只要在代码里集成对应的 JS 版客户端协议库                    | 完成 |
| Kafka   | connect()                         | 只要在代码里集成对应的 JS 版客户端协议库                    | 计划 |
...
为了在 Cloudflare Workers 免费版的 10ms CPU 限制内运行，本项目采用**轮流检测模式**：

- 每分钟只检测 **1 个站点**
- 站点按顺序轮流检测
- N 个站点 = 每 N 分钟完成一轮完整检测

| 监控类型 | 检测方式 | 检测频率 | 说明 |
|----------|----------|----------|------|
| HTTP/HTTPS | Cron 轮流检测 | 每站点 N 分钟 | N = 站点总数 |
| DNS | Cron 轮流检测 | 每站点 N 分钟 | 通过 1.1.1.1 DoH API |
| TCP | Cron 轮流检测 | 每站点 N 分钟 | 通过 Cloudflare Sockets |
| Push 心跳 | 被动接收 | 实时 | 主机主动上报，不受轮流限制 |
| SSL 证书 | 异步检测 | 每小时 | 证书变化慢，无需频繁检测 |
...
历史记录使用关系型表存储，彻底解决 CPU 超时问题：

| 操作 | 复杂度 | 说明 |
|------|--------|------|
| 写入 | O(1) | 直接 INSERT，无需解析现有数据 |
| 读取 | O(log N) | 数据库索引查询 |
| 清理 | O(1) | DELETE WHERE 语句 |
...
| 30 个站点 | 30 次 | ~43,200 次 | 43.2% |
| 50 个站点 | 50 次 | ~72,000 次 | 72% |
| 60 个站点 | 60 次 | ~86,400 次 | 86.4% |

> **提示**：1 分钟检测间隔下，免费版可支持约 60 个站点。如需监控更多站点，可考虑升级到付费版。
