...
| **新增后台设置页面** | 你现在可以在后台设置页面修改密码和后台路径 |
...
- **修改密码**：输入当前密码和新密码，点击修改。密码将自动使用 SHA-256 加密后存储。
- **修改后台路径**：输入新的后台路径，点击修改。修改后将自动跳转到新地址。
...
4. 添加或编辑键值：
   ...
   - 修改后台路径：Key = `admin_path`，Value = `新路径`（不带斜杠，如 `my-admin`）
5. 保存后立即生效
